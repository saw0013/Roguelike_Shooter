...
public class RagdollBuilder : ScriptableWizard
{
    ...
    [Header("--- Аниматор персонажа cursorObject ---")]
    public Animator animator;
    [Header("--- Общий шаблон персонажа ---")]
    ...
    [Header("Общая масса будет проигнорирована и будет установлена на 1, если пропорциональная масса истинна.")]
    ...
    string CheckConsistency()
    {
        ...
        foreach (BoneInfo bone in bones)
        {
            if (bone.anchor)
            {
                if (map[bone.anchor] != null)
                {
                    ...
                    return String.Format("{0} и {1} не могут быть назначены одной и той же кости.", bone.name, oldBone.name);
                }
        ...
        foreach (BoneInfo bone in bones)
        {
            if (bone.anchor == null)
            {
                return String.Format("{0} еще не назначен.\n", bone.name);
            }
        }
    ...
    [MenuItem("Tools/Cosmoground/Ragdoll")]
    static void CreateWizard()
    {
        ScriptableWizard.DisplayWizard("Создание Ragdoll", typeof(RagdollBuilder));
    ...
    void Update()
    {
        ...
        {
            helpString = "Перетащите все кости из иерархии в их слоты.\nУбедитесь, что ваш персонаж находится в Т-позе.\n";
        }
        else
        {
            helpString = "Убедитесь, что ваш персонаж находится в Т-образной позу.\nУбедитесь, что синяя ось направлена в том же направлении, что и персонаж.\nИспользуйте flipForward, чтобы изменить направление";
        }
