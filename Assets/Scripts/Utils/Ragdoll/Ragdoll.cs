...
public class Ragdoll : MonoBehaviour
{
    ...
    public void LoadBodyPart()
    {
        ...
        //Ошибка. Ссылка на объект не указывает на экземпляр
    ...
    /// <summary>
    /// Сбросить переменную inApplyDamage можно установив значение false;
    /// </summary>
    ...
    /// <summary>
    /// Сбросить сохранённое время ragdoll
    /// </summary>
    ...
    /// <param name="timeToStayRagdolled">Время пропуска ragdolled </param>
    ...
    // Вызовать этот метод если нужен Ragdoll
    public void ActivateRagdoll(Damage damage)
    {
        ...
        // Начать преврку, стабилен ли Ragdoll
    ...
    // Стабилизатор ragdoll - подождите, пока ragdoll станет стабильный в зависимости от velocity.magnitude
    ...
    // reset player - восстановить контроль над персонажем
    ...
    // ragdoll blend - код, основанный на скрипте Perttu H?m?l?inen с модификациями для работы с этим контроллером      
    void RagdollBehaviour()
    {
        ...
        //Переход от ragdoll обратно к анимированному
        if (state == RagdollState.blendToAnim)
        {
            if (Time.time <= ragdollingEndTime + mecanimToGetUpTransitionTime)
            {
                //Если мы ждем, пока Mecanim начнет воспроизводить анимацию вставания, обновим корень mecanim
                //персонажа для наилучшего совпадения с рэгдоллом
                ...
                //Теперь проводим луч из рассчитанной позиции вниз и находим наивысшее попадание, которое не принадлежит персонажу
                ...
                //Получить ориентацию тела в плоскости земли как для позы рэгдолла, так и для анимированной позы вставания
                ...
                //Попробуйте сопоставить повороты. Обратите внимание, что мы можем вращаться только вокруг оси Y, так как анимированный персонаж должен оставаться в вертикальном положении.
                // следовательно, устанавливаем компоненты y векторов равными нулю.
